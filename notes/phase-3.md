...
- Logging into the admin panel now also sets the display cookie, so a new phone needs no long display link: log in, and the home page shows the QR there. It also keeps the organizer's own device signed in after they make a new display link. The organizer is allowed everything a display is, so this grants nothing new.
